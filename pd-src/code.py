...
def render_full_from_bits(frame_bits: bytes) -> None:
    for y in range(HEIGHT):
        ...
        base_fb = y * WIDTH  # Changed from * 2
        for byte_index in range(BYTES_PER_ROW):
            ...
            for k in range(num_pixels):
                dst_pos = base_fb + (byte_index * 8 + k)  # Changed from * 2
                pixel_on = (b >> (7 - k)) & 1

                # Write single byte: 0xFF for white, 0x00 for black
                buf[dst_pos] = 0xFF if pixel_on else 0x00
